...
The vital activity of the sense-organ and the object of perception that acts upon it jointly cause an effect received in the sense-organ. Following Theophrastus, Priscian describes this effect as a motion. The motion, the joint effect of the external form and the activity that it aroused in the sense-organ, is a likeness <\emph{homoiôma}> of the sensible form of the external body. This likeness constitutes an \emph{emphasis} of the perceived object. It can mean appearance or reflection or even an appearance in a reflection. Huby in \citet[51 n.25]{Sorabji:1997ly} translates \emph{emphasis} as ``representative image''. It does assume a technical significance in Priscian's treatise. And nothing like what we would ordinarily describe as appearance has been achieved in the first moment (perhaps only in the second). Nor is Priscian explaining perception in terms of reflection. So perhaps Huby's non-committal rendering is apt.

Already in the first moment, the most passive moment, in the process of formal assimilation, Priscian is keen to emphasize the activity of the sensitive soul and so minimize the role of passive affection from without: ``Hence neither is the whole thing <the motion> a passive effect nor is it entirely from outside, but is is also by way of <the senses> own activity'' (Priscian, \emph{Metaphrais} 2.4; Huby in \citealt[10]{Sorabji:1997ly}). Indeed, the joint determinants of the motion in the sense-organ that is a likeness of the external form are carefully counterposed: 
\begin{quote}
...
\end{quote}
The first moment in the process of formal assimilation is conceived by Priscian to be a mode of arousal.
...
\section{Two Related Problems \ldots} % (fold)
\label{sec:some_related_problems_ldots}

\subsection{Plotinus} % (fold)
\label{sub:plotinus}

Plotinus inagurates \emph{Ennead} 3.6 with an \emph{aporia}:
\begin{quote}
...
Before considering how Priscian would address this problem, let us first consider a distinct, if related problem raised by Aquinas:
\begin{quote}
...
% This would be a return to the previous opinion which held that the knowledge of all things is naturally present in the human soul. 
% TODO chase up the larger argument here
